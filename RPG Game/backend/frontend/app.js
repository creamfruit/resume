...
function renderItemDetails(item = null) {
  ...
  const compareText = equipped
    ? `Equipped ${i.slot}: ${equipped.name} (${equippedPower}) | Delta: ${deltaSign}${delta}`
    : `Equipped ${i.slot}: None | Delta: +${Number(i.power || 0)}`;
...
async function renderRunesTab() {
  ...
  const rarityRows = rarityOrder.map((r) => {
    const count = inv.filter((x) => String(x.rarity || '').toLowerCase() === r).length;
      ...
      <article class="home-card">
        ...
        <div class="rune-slot-grid">
          ${Array.from({ length: cap }).map((_, i) => {
            ...
            const rune = inv.find((x) => String(x.id || '') === String(rid));
            return `
