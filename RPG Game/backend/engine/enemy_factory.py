...
MODIFIER_POOL = {
    "colossal": {"hp": 1.18, "atk": 1.05, "def": 1.05},
    "volatile": {"hp": 0.96, "atk": 1.16, "def": 0.92},
    "runic": {"hp": 1.04, "atk": 1.10, "def": 1.06},
    "swift": {"hp": 0.92, "atk": 1.08, "def": 0.95},
}
...
def _roll_modifiers(depth: int, risk: int, rng: random.Random) -> list[str]:
    if depth <= 1 and risk <= 0:
        pool = ["colossal", "volatile", "runic"]
        rng.shuffle(pool)
        return pool[: rng.randint(2, 3)]

    pool = list(MODIFIER_POOL.keys())
    rng.shuffle(pool)
    count = 1 if depth <= 2 else rng.randint(1, 2)
    return pool[:count]
...
def create_enemy(depth: int, risk: int) -> Enemy:
    ...
    modifiers = _roll_modifiers(depth=depth, risk=risk, rng=rng)
    for mod in modifiers:
        blend = MODIFIER_POOL[mod]
        hp = int(hp * blend["hp"])
        atk = int(atk * blend["atk"])
        defense = int(defense * blend["def"])
        crit = round(min(0.18, crit + 0.02), 3)
