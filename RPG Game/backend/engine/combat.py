...
from engine.boss_ai import is_boss, roll_boss_intent, roll_enemy_intent
...
def _counter_for_intent(intent: Dict | None) -> str:
    if not isinstance(intent, dict):
        return "basic"
    if intent.get("type") == "heavy":
        return "guard"
    if intent.get("type") == "multi":
        return "rupture"
    if intent.get("type") == "guard_break":
        return "focus"
    return "heavy"


def _auto_battle_response(player: Any, enemy: Any) -> str:
    intent = getattr(enemy, "intent", None)
    if not isinstance(intent, dict):
        intent = roll_enemy_intent(enemy, risk=int(getattr(player, "depth", 1) or 1))
    return _counter_for_intent(intent)


def _resolve_response(player: Any, enemy: Any, response: str | None = None, auto_battle: bool = False) -> Dict[str, Any]:
    intent = getattr(enemy, "intent", None)
    if not isinstance(intent, dict):
        intent = roll_enemy_intent(enemy, risk=int(getattr(player, "depth", 1) or 1))

    if auto_battle:
        response = _auto_battle_response(player, enemy)

    response = str(response or "").lower().strip() or "basic"
    counter = str(intent.get("counter", "basic") or "basic").lower().strip()
    matched = response == counter

    mods = getattr(enemy, "combat_mods", {}) or {}
    if matched:
        mods["exposed"] = {"turns": 1, "damage_mult": 1.4}
        enemy.combat_mods = mods
    else:
        mods.pop("exposed", None)
        enemy.combat_mods = mods
    ...
    return {
        ...
        "matched": matched,
        "intent": intent,
        "telegraph": str(intent.get("telegraph", "") or ""),
        "description": str(intent.get("description", "") or ""),
    }


def sync_enemy_intent(player: Any, enemy: Any) -> Dict[str, Any]:
    intent = getattr(enemy, "intent", None)
    if not isinstance(intent, dict):
        intent = roll_enemy_intent(enemy, risk=int(getattr(player, "depth", 1) or 1))
    return intent
...
def player_attack(player: Any, enemy: Any, rng_seed: int | None = None, response: str | None = None, auto_battle: bool = False) -> Dict:
    rng = random.Random(rng_seed)
    passives = collect_equipment_passives(player)
    _clear_temp_mods(player)
    _clear_temp_mods(enemy)

    intent = sync_enemy_intent(player, enemy)
    response_state = _resolve_response(player, enemy, response=response, auto_battle=auto_battle)
    ...
    if isinstance(emods.get("exposed"), dict):
        dmg_bonus = float(emods.get("exposed", {}).get("damage_mult", 0.0) or 0.0)
        if dmg_bonus > 0:
            pmods["damage_mult"] = float(pmods.get("damage_mult", 0.0) or 0.0) + (dmg_bonus - 1.0)
            player.combat_mods = pmods

    derived = compute_derived_stats(player)
    dmg = float(derived.get("attack", 0.0) or 0.0)
...
def enemy_attack(player: Any, enemy: Any, dodge_success: bool, rng_seed: int | None = None, response: str | None = None, auto_battle: bool = False) -> Dict:
    rng = random.Random(rng_seed)
    passives = collect_equipment_passives(player)
    _clear_temp_mods(player)
    _clear_temp_mods(enemy)
    ...
    if is_boss(enemy):
        ...
        if not isinstance(intent, dict):
            intent = roll_enemy_intent(enemy, risk=int(getattr(player, "depth", 1) or 1), rng_seed=rng.randint(1, 10_000_000))

    hits = int(intent.get("hits", 1) or 1)
    dmg_mult = float(intent.get("damage_mult", 1.0) or 1.0)
    if response_state.get("matched"):
        dmg_mult *= 0.55
        emods = getattr(enemy, "combat_mods", {}) or {}
        emods["exposed"] = {"turns": 0, "damage_mult": 1.0}
        enemy.combat_mods = emods
