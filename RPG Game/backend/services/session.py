...
INTENT_LIBRARY = {
    "basic": {
        ...
        "telegraph": "A measured strike is coming.",
        "counter_action": "heavy",
    },
    "heavy": {
        "type": "heavy",
        "name": "Brutal Swing",
        "telegraph": "A slow heavy strike is winding up.",
        "counter_action": "guard",
    },
    "multi": {
        "type": "multi",
        "name": "Rapid Combo",
        "telegraph": "The enemy shifts into a rapid combo stance.",
        "counter_action": "rupture",
    },
    "guard_break": {
        "type": "guard_break",
        "name": "Guard Break",
        "telegraph": "A guard-breaking blow is being prepared.",
        "counter_action": "focus",
    },
}
...
def _seed_enemy_intent(enemy: Enemy, risk: int):
    if is_boss(enemy):
        if not isinstance(getattr(enemy, "intent", None), dict):
            enemy.intent = roll_boss_intent(enemy, rng_seed=random.randint(1, 10_000_000))
        return
...
def roll_enemy_intent(enemy: Enemy, risk: int = 0) -> Dict[str, Any]:
    if is_boss(enemy):
        return roll_boss_intent(enemy, rng_seed=random.randint(1, 10_000_000))

    arch = str(getattr(enemy, "archetype", "brute") or "brute").lower()
    deck = ARCHETYPE_INTENT_DECKS.get(arch, ARCHETYPE_INTENT_DECKS["brute"])

    mods = getattr(enemy, "combat_mods", {}) or {}
    idx = int(mods.get("intent_index", random.randint(0, len(deck) - 1)) or 0)
    pressure = mods.get("room_pressure", {}) if isinstance(mods.get("room_pressure"), dict) else {}
    step = (2 if risk >= 4 else 1) + int(pressure.get("intent_shift", 0) or 0)
    intent_key = deck[idx % len(deck)]
    mods["intent_index"] = (idx + step) % len(deck)
    enemy.combat_mods = mods

    intent = dict(INTENT_LIBRARY.get(intent_key, INTENT_LIBRARY["basic"]))
    intensity = float(pressure.get("intensity", 0.0) or 0.0)
    if intensity > 0:
        intent["damage_mult"] = round(float(intent.get("damage_mult", 1.0) or 1.0) * (1.0 + (intensity * 0.45)), 3)
        if intent_key in {"heavy", "guard_break"} and intensity >= 0.20:
            intent["telegraph"] = f"{intent.get('telegraph', '')} Pressure spike building."
        elif intensity >= 0.14:
            intent["telegraph"] = f"{intent.get('telegraph', '')} Pace is accelerating."
        intent["pressure"] = {
            "label": str(pressure.get("label", "Low") or "Low"),
            "intensity": intensity,
        }
    enemy.intent = intent
    return intent
