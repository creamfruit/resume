...
from services.stash import get_stash
from services.equipment import equip_item, unequip_item
from services.dungeon_run import ROOM_AFFIXES, run_dungeon, start_interactive_dungeon, complete_interactive_dungeon
...
from ai.openai_client import has_api_key
...
RUNE_CHEST_WEIGHTS = {
    ...
    "epic": 42,
    "legendary": 10,
    # Chest drops are capped at legendary; higher tiers come from combining.
    "mythic": 0,
    "supreme": 0,
    "relic": 0,
}
...
def _item_from_payload(payload: dict):
    if hasattr(Item, "model_validate"):
        return Item.model_validate(payload or {})
    return Item(**dict(payload or {}))
...
def _apply_cursed_skill(player: Player, enemy: Enemy, rolled_skill: dict) -> dict:
    ...
    curse_tune = int(player.battle_tree.get("curse_attunement", 0) or 0)
    stamina_gain = 4 + max(0, curse_tune * 2)
    if mastery_tier >= 1:
        stamina_gain += 2
    player.stamina = min(float(player.max_stamina), float(player.stamina) + stamina_gain)
    ...
    if sid == "self_bleed":
        ...
        if mastery_tier >= 1:
            stamina_bonus = 4
            player.stamina = min(float(player.max_stamina), float(player.stamina) + stamina_bonus)
            perks.append("insight_stamina")
        if mastery_tier >= 3:
            perks.append("blank_reroll")
        detail = {"effect": "insight_reroll", "reroll_gain": reroll_gain, "stamina_gain": stamina_bonus}
    ...
    return {
        ...
        "stamina_after": float(player.stamina),
...
def _rune_slot_capacity(player: Player) -> int:
    ...
    return int(player.rune_slot_capacity)
...
def _roll_rune_rarity(luck_bonus: float = 0.0) -> str:
    ...
    # relic from chest is effectively near-impossible
...
def _generate_build_rune(player: Player) -> dict:
    rarity = _roll_rune_rarity(player.loot_luck)
    ...
    return {
        ...
        "name": name,
        "rarity": rarity,
        "effects": _generate_rune_effects(rarity),
        "source": "chest",
        "upgrade_level": 0,
...
def rune_equip(payload: dict):
    ...
    # one rune cannot be equipped in multiple slots
...
def rune_combine_by_rarity(payload: dict):
    ...
    pool = [r for r in current_player.rune_items if str(r.get("rarity", "")).lower() == rarity]
    if len(pool) < 4:
...
# -------------------------
# Dungeon - AUTO MODE (kept)
# -------------------------
@app.get("/dungeon/run")
def dungeon_run(risk: int = 0):
    return run_dungeon(current_player, risk=risk)
...
def _apply_intent_counter(player: Player, enemy: Enemy, action: str, player_combat: dict):
    ...
    if action == counter_action:
        player.stamina = min(player.max_stamina, player.stamina + 10)
        player_combat["counter"] = {
            "success": True,
            "against": intent.get("type", "unknown"),
            "reward": "stamina_refund+damage_bonus",
        }
        dmg = float(player_combat.get("damage", 0.0) or 0.0)
        if dmg > 0:
...
def _run_player_phase(action: str, use_reroll: bool = False):
    enemy = session.current_enemy()
    ...
    _apply_battle_defaults(current_player)
    rolled_skill = _roll_battle_skill(current_player, use_reroll=bool(use_reroll))
    mastery_gain = _battle_mastery_gain(
        current_player,
        str(rolled_skill.get("id", "") or ""),
        7.0 + (float(session.SESSION.get("risk", 0) or 0) * 1.6),
    )
    action = str(rolled_skill.get("action", "") or "").lower().strip()
    ...
    if str(rolled_skill.get("kind", "normal")) == "cursed":
        ...
        # If rolled action is not currently usable, fallback to a basic strike.
        if player_combat.get("error"):
            fallback = _player_action_turn(current_player, enemy, "basic", damage_multiplier=max(0.75, damage_mult * 0.8))
            if not fallback.get("error"):
                fallback["battle_fallback"] = {
                    "reason": player_combat.get("error", "rolled_action_unusable"),
                    "rolled_action": action,
                }
                player_combat = fallback
                action = "basic"
    ...
    if str(rolled_skill.get("kind", "normal")) != "cursed":
        ...
        if mastery_tier >= 1:
            before = float(current_player.stamina)
            current_player.stamina = min(float(current_player.max_stamina), float(current_player.stamina) + 4.0)
            gained = round(float(current_player.stamina) - before, 2)
            if gained > 0:
                mastery_perks.append({"id": "stamina_refund", "value": gained})
    ...
    if player_combat.get("error"):
        return {
            ...
            "allowed_actions": list(ACTION_CONFIG.keys()),
            "resources": {
                "stamina": current_player.stamina,
                "max_stamina": current_player.max_stamina,
                "action_cooldowns": current_player.action_cooldowns,
    ...
    if str(rolled_skill.get("kind", "normal")) == "cursed":
        session.add_log(
            f"Player rolls CURSED skill {str(rolled_skill.get('name', rolled_skill.get('id', 'unknown'))).upper()}"
        )
    elif action in ("guard", "focus"):
        session.add_log(f"Player uses {action.upper()} (stamina {current_player.stamina}/{current_player.max_stamina})")
    else:
        skill_name = str(rolled_skill.get("name", action) or action).upper()
        session.add_log(f"Player rolls {skill_name} -> {action.upper()} for {player_dmg} (enemy hp now {max(enemy.hp, 0)})")
...
def _run_enemy_phase(dodge_success: bool):
    ...
    _regen_stamina(current_player, amount=20)
    ...
    if active_prayer == "mystic_will" and _consume_prayer_runes(current_player, active_prayer):
        _regen_stamina(current_player, amount=int(PRAYER_BOOK[active_prayer].get("value", 8)))
        enemy_combat["prayer_stamina_boost"] = int(PRAYER_BOOK[active_prayer].get("value", 8))
    if is_boss(enemy):
        _update_boss_run_state(enemy)
    _seed_next_enemy_intent(enemy)
    enemy_combat["next_intent"] = getattr(enemy, "intent", None)
    session.add_log(
        f"{enemy.name} attacks | dmg {enemy_combat.get('damage', 0)} | player hp {current_player.hp}"
    )
...
def player_phase(payload: dict):
    ...
    return _run_player_phase(action, use_reroll=use_reroll)
