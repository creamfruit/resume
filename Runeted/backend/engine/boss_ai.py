...
INTENT_LIBRARY = {
    ...
    "heavy": {
        "type": "heavy",
        "name": "Brutal Swing",
        "description": "A slow heavy strike is winding up.",
        "counter": "guard",
    },
    "multi": {
        "type": "multi",
...
def _telegraph_from_template(intent_key: str, pressure: float = 0.0) -> Dict:
    base = dict(INTENT_LIBRARY.get(intent_key, INTENT_LIBRARY["basic"]))
    idx = base["type"]
    name = base["name"]
    description = base["description"]
    if pressure > 0.0:
        description = f"{description} Pressure is rising."
    if idx == "multi":
        return {
            **base,
            "name": name,
            "description": description,
            "effect": {"kind": "multi_hit", "damage_mult": 1.0 + pressure * 0.45},
            "counter": base.get("counter", "rupture"),
        }
    if idx == "heavy":
        return {
            **base,
            "name": name,
            "description": description,
            "effect": {"kind": "heavy_damage", "damage_mult": 1.5 + pressure * 0.35},
            "counter": base.get("counter", "guard"),
        }
    if idx == "guard_break":
        return {
            **base,
            "name": name,
            "description": description,
            "effect": {"kind": "guard_break", "damage_mult": 1.2 + pressure * 0.25},
            "counter": base.get("counter", "focus"),
        }
    return {
        **base,
        "name": name,
        "description": description,
        "effect": {"kind": "basic_damage", "damage_mult": 1.0 + pressure * 0.2},
        "counter": base.get("counter", "heavy"),
    }


def roll_enemy_intent(enemy: Any, risk: int = 0, rng_seed: int | None = None) -> Dict:
    if is_boss(enemy):
        return roll_boss_intent(enemy, rng_seed=rng_seed)

    arch = str(getattr(enemy, "archetype", "brute") or "brute").lower()
    deck = ARCHETYPE_INTENT_DECKS.get(arch, ARCHETYPE_INTENT_DECKS["brute"])
    mods = getattr(enemy, "combat_mods", {}) or {}
    if not isinstance(mods, dict):
        mods = {}
    rng = random.Random(rng_seed)
    idx = int(mods.get("intent_index", rng.randint(0, len(deck) - 1)) or 0)
    intent_key = deck[idx % len(deck)]
    mods["intent_index"] = (idx + 1 + max(0, int(risk) // 3)) % len(deck)
    enemy.combat_mods = mods

    pressure = float((mods.get("room_pressure") or {}).get("intensity", 0.0) or 0.0)
    intent = _telegraph_from_template(intent_key, pressure=pressure)
    intent["telegraph"] = intent.get("description", "")
    enemy.intent = intent
    return intent
...
def roll_boss_intent(enemy: Any, rng_seed: int | None = None) -> Dict:
    ...
    if choice == "basic":
        intent = {
            "type": "basic",
            "name": "Basic Attack",
            "description": "The boss raises its weapon.",
        ...
        intent = {
            "type": "heavy",
            "name": "Heavy Slam",
            "description": "Charging a heavy slam... (harder to dodge)",
        ...
        intent = {
            "type": "multi",
            "name": "Flurry",
            "description": "A fast flurry is coming... (multiple hits)",
    ...
    if mood == "predatory":
        ...
        intent["description"] = f"{intent.get('description', '')} The boss is feeding on your momentum."
    elif mood == "hunting":
        intent["telegraph"] = f"{intent.get('telegraph', '')} The boss senses an opening."
        intent["description"] = f"{intent.get('description', '')} The boss senses an opening."
    elif mood == "desperate":
        intent["telegraph"] = f"{intent.get('telegraph', '')} Desperation is pushing the tempo."
        intent["description"] = f"{intent.get('description', '')} Desperation is pushing the tempo."
