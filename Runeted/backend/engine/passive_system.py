...
def collect_equipment_passives(player: Any) -> List[PassiveModel]:
    ...
    if weapon:
        ...
        passives += normalize_passives(getattr(weapon, "innate_abilities", None))
    if armor:
    ...
    if isinstance(eq, dict):
        for slot in ("weapon", "armor", "helm", "ring", "amulet", "boots", "gloves"):
            ...
            if isinstance(item, dict):
                ...
                passives += normalize_passives(item.get("innate_abilities"))
            else:
                passives += normalize_passives(getattr(item, "passives", None) if item else None)
                passives += normalize_passives(getattr(item, "innate_abilities", None) if item else None)
