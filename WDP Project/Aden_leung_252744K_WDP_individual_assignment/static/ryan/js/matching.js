...
async function apiDeleteMatch(matchId) {
  const res = await fetch(`/ryan/api/matches/${encodeURIComponent(matchId)}`, { method: "DELETE" });
...
async function apiClearMatches() {
  const res = await fetch("/ryan/api/matches", { method: "DELETE" });
...
function renderMatchesGrid(matches) {
  ...
  matches.forEach((m) => {
    ...
    card.querySelector(".matches-cta").addEventListener("click", () => {
      window.location.href = `/ryan/messages?chat=${encodeURIComponent(m.match_id)}`;
    });
...
if (sendMessageBtn) {
  sendMessageBtn.addEventListener("click", () => {
    ...
    if (chatId) window.location.href = `/ryan/messages?chat=${encodeURIComponent(chatId)}`;
    else window.location.href = "/ryan/messages";
  });
}
