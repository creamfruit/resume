...
async function loadAchievements() {
    const res = await fetch('/ryan/api/achievements');
    if (!res.ok) return;
...
function renderLandmarks() {
    ...
    landmarksData.forEach((landmark, index) => {
        ...
        g.setAttribute('class', `landmark-marker $shisLocked ? 'locked' : ''} ${isCurrent ? 'current' : ''}`);
...
function unlockLandmark(index) {
    ...
    if (DB_MODE) {
        fetch(`/ryan/api/achievements/landmarks/${landmark.id}/unlock`, {method: 'POST'})
            .then(res => res.json())
            .then(data => {
                if (!data || !data.ok) {
...
function checkAnswer(landmarkIndex, answerIndex) {
    ...
    if (answerIndex === landmark.answer) {
        ...
        if (DB_MODE) {
            fetch(`/ryan/api/achievements/landmarks/${userLandmark.id}/complete`, {method: 'POST'})
                .then(res => res.json())
                .then(data => {
                    if (!data || !data.ok) {
    ...
    userData.quests.forEach(quest => {
        ...
        questCard.innerHTML = `
            <div class="card-body">
                ...
                <button class="btn btn-orange btn-sm" type="button" onclick="completeQuest(${quest.id})" ${isCompleted ? 'disabled' : ''}>
                    ${isCompleted ? 'Completed \u{2705}' : 'Start Quest'}
                </button>
            </div>
        `;
        if (isCompleted) {
...
function completeQuest(questId) {
    ...
    if (DB_MODE) {
        fetch(`/ryan/api/achievements/quests/${questId}/progress`, {method: 'POST'})
            .then(res => res.json())
            .then(data => {
                if (!data || !data.ok) {
...
async function redeemReward(rewardId) {
    ...
    if (DB_MODE) {
        const res = await fetch(`/ryan/api/achievements/rewards/${rewardId}/redeem`, {method: 'POST'});
        const data = await res.json().catch(() => null);
        if (!data || !data.ok) {
...
async function updateSkillProgress(skillId) {
    ...
    if (DB_MODE) {
        const res = await fetch(`/ryan/api/achievements/skills/${skillId}/progress`, {method: 'POST'});
        const data = await res.json().catch(() => null);
        if (!data || !data.ok) {
